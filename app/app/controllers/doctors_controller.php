...
class DoctorsController extends AppController {
  ...
  function byName($name) {
	...
	$this->set('minuteInterval', 15);
	
	//Only coming up with one
	$totalLocations = count($this->Doctor->DoctorDetail->find('All'));
	$this->set('totalLocations', $totalLocations);
	
	$totalReviews = count($this->Doctor->DoctorReview->findByName($name));
